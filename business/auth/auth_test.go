...
func TestAuth(t *testing.T) {
	t.Log("Given the need to be able to authenticate and authorize access.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single user.", testID)
		{
			...
			if ok := claims.Authorized(auth.RoleAdmin); !ok {
				t.Errorf("\t%s\tTest %d:\tShould yafdadadfa: %v", failed, testID, err)
			} else {
				t.Logf("\t%s\tTest %d:\tShould yafdadadfa: %v", success, testID, err)
			}
		}
	}
}
